...
description: >-
  ...
  代码现状）。沿用 001-provider / 002-react 事实模板的全部约定。课件与四文档冲突时停下报告
  等用户拍板，不得自行仲裁。
...
6. **完成报告**：产物路径 + 「为实现级明确」决策清单 + 超出四文档的新增对外概念（如有）
   + 冲突拍板记录（课件 vs 四文档，如有）+ 前序缺口（H0 依赖检查发现，如有）
...
- **课件与四文档（或宪法）冲突 → 停下报告，不得自行仲裁**（Harness H1 同款纪律）：
  列出冲突点清单（课件说法 vs 文档说法），用 AskUserQuestion 等用户拍板；拍板结论写入
  文档头部「修订说明」，并在完成报告中记录。历史案例：002-react 经用户拍板"冲突一律
  参照课件"（唯一例外：课件 skills 字段表述 vs 宪法 IV 软连接，按宪法）。
