...
| FR-2 | **三块白名单配置**：`FileSandboxProperties`（prefix `file`，`allowedPaths`）、`ShellSandboxProperties`（prefix `shell`，`allowedCommands`）、`HttpSandboxProperties`（prefix `http`，`allowedDomains`）三个 `@ConfigurationProperties` record，装配处 `@EnableConfigurationProperties` 注册（G4-C1 无组件注解，005 拍板延续）；对应 `application.yaml` 配置键 `file.allowed_paths` / `shell.allowed_commands` / `http.allowed_domains`（CLAUDE.md 原则六、技术方案 §6.7、课件 24 §3.2 三处一致）。**空配置 = 什么都不允许，而非"不校验"**（课件 24 §五人工项点名的口径；`Set.copyOf` 空集下 `contains` 恒 false 自然满足，测试钉死） | `FileSandboxProperties` / `ShellSandboxProperties` / `HttpSandboxProperties`（oryxos-tool）+ `application.yaml` 三个配置键 | 课件 24 §3.2/§五；CLAUDE.md 原则六 |
| FR-3 | **`checkFilePath`：路径标准化 + 前缀匹配**——`Path.of(rawPath).normalize().toAbsolutePath()` 后 `allowedRoots.stream().anyMatch(target::startsWith)`；白名单根本身也经 `Path.of(...).normalize()` 预处理（构造期）。**`../` 路径穿越由 normalize 吸收**（`/workspace/../../outside/secret.txt` → 白名单外 → 拒绝，最值钱回归测试钉死） | `WhitelistSandbox.checkFilePath`（私有） | 课件 24 §3.2/§四 |
| FR-4 | **`checkShellCommand`：首 token 白名单**——`command.trim().split("\\s+")[0]` 取首 token 精确匹配（**大小写敏感**——LS ≠ ls 拒绝，课件骨架无归一化、按精确匹配实现级明确；**前导空格经 trim 容忍**，课件 harness 表点名的处理） | `WhitelistSandbox.checkShellCommand`（私有） | 课件 24 §3.2/§四；需求文档 §5.6「命令白名单」 |
...
public class WhitelistSandbox implements Sandbox {
  ...
  private final List<Path> allowedRoots;         // 构造期 normalize
  ...
  public WhitelistSandbox(FileSandboxProperties fileProps,
    ...
    this.allowedRoots = fileProps.allowedPaths().stream()
        .map(Path::of).map(Path::normalize).toList();
...
> 来源：课件 23（§四 劝阻级 / §八 纵深防御 / §九 Hermes 教训 / §十三 覆盖面 / §十四~§十五 分阶段与信号）、技术方案 §6.7、需求文档 §5.6、宪法 VI。
...
| 测试类 | 关键回归点 |
|--------|-----------|
| `WhitelistSandboxTest`（文件路径组） | 白名单内放行 / 白名单外拒绝 / **相对路径穿越被拦**（`/workspace/../../outside/secret.txt` → SandboxViolationException，normalize 回归——课件原文用例）；白名单根构造期 normalize |
| `WhitelistSandboxTest`（Shell 命令组） | 白名单内放行（`ls`）/ 白名单外拒绝（`rm`）/ **首 token 前导空格容忍**（`"  ls -la"` → trim 后放行）/ **大小写敏感**（`LS` 拒绝，精确匹配口径）；空命令拒绝 |
...
跑法：`mvn test` 日常全跑；全量 `mvn clean verify` 收尾（342 tests + 本节新增全绿）。
